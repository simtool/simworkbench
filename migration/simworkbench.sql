...
CREATE TABLE `task` (
    ...
    `expiration_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '任务过期时间',
    `creator_id` char(24) NOT NULL COMMENT '任务创建人 ID',
    `executor_id` char(24) NOT NULL COMMENT '任务执行人 ID',
    `completed` tinyint(1) NOT NULL DEFAULT 0 COMMENT '已完成标识，0：未完成，1：已完成',
    `parent_id` char(24) NOT NULL DEFAULT '000000000000000000000000' COMMENT '主任务 ID，当前任务为主任务时值为：000000000000000000000000',
    `project_id` char(24) NOT NULL COMMENT '任务所属项目 ID',
    `column_id` char(24) NOT NULL COMMENT '任务所属列 ID',
    `sequence` int(11) NOT NULL COMMENT '任务在所属列中的排序',
    `comment` text NOT NULL COMMENT '任务评论',
...
CREATE TABLE `project` (
    ...
    `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '项目更新时间',
    `creator_id` char(24) NOT NULL COMMENT '项目创建人 ID',
    `deleted` tinyint(1) NOT NULL DEFAULT 0 COMMENT '软删除标识，0：未删除，1：已删除',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `user` (
    ...
    `weekly_newspaper_receiver` text NOT NULL COMMENT '周报收件人',
    `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '项目创建时间',
    `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '项目更新时间',
    `deleted` tinyint(1) NOT NULL DEFAULT 0 COMMENT '软删除标识，0：未删除，1：已删除',
    PRIMARY KEY (`id`)
...
CREATE TABLE `project_user` (
    ...
    `project_id` char(24) NOT NULL COMMENT '项目 ID',
